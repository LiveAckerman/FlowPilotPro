...
test('phone auth detects Chinese "发送文本消息" wording on contact-verification page', () => {
  const originalLocation = global.location;
  const originalDocument = global.document;
  try {
    global.location = {
    ...
    const helpers = api.createPhoneAuthHelpers({
      fillInput: () => {},
      getActionText: () => '',
      getPageTextSnapshot: () => '无法向此电话号码发送文本消息，请尝试其他号码。',
      getVerificationErrorText: () => '',
      humanPause: async () => {},
      isActionEnabled: () => true,
      isAddPhonePageReady: () => false,
    ...
    assert.equal(result.reason, 'resend_phone_banned');
    assert.equal(result.prefix, 'PHONE_RESEND_BANNED_NUMBER::');
    assert.match(result.message, /无法向此电话号码发送文本消息/);
  } finally {
    global.location = originalLocation;
    global.document = originalDocument;
  }
