  ...
  function createMessageRouter(deps = {}) {
    ...
    async function handleMessage(rawMessage, sender) {
      ...
      switch (message.type) {
        ...
        case 'FETCH_HERO_SMS_TOP_STATS': {
          // 给侧栏「获取热门便宜 HeroSMS 号码」按钮用。
          //
          // 关键点：/api/v1/stats/deliverability 用 hero-sms.com 的 session cookie 鉴权。
          // MV3 service worker 的 fetch(credentials:'include') 默认**不会**带 SameSite=Lax/Strict
          // 的 cookie（即使有 host_permissions），所以后台直接 fetch 永远 401。
          //
          // 正确做法：找一个已打开的 hero-sms.com 标签页，用 chrome.scripting.executeScript
          // 把 fetch 注入到**页面上下文**里跑，浏览器走同源策略自动带 cookie。
          // 如果一个都没开，就主动开一个新 tab 等加载完再注入，注入完返回数据。
          const stateForLookup = await getState();
          const apiKey = String(stateForLookup?.heroSmsApiKey || '').trim();
          const service = String(message.payload?.service || 'dr').trim() || 'dr';
          const interval = Number(message.payload?.interval) === 24 ? 24 : 12;
          const successCount = ['low', 'medium', 'high'].includes(String(message.payload?.successCount || '').trim())
            ? String(message.payload.successCount).trim()
            : 'medium';

          if (!apiKey) {
            return {
              error: 'HeroSMS API Key 未配置，请先在侧边栏填写后再试。',
              code: 'NO_API_KEY',
            };
          }

          const TARGET_URL = 'https://hero-sms.com/cn/statistics/top10?service=dr';
          const HERO_HOSTNAME_PATTERN = /^https?:\/\/(?:[a-z0-9-]+\.)*hero-sms\.com\//i;

          async function findHeroTab() {
            try {
              const tabs = await chrome.tabs.query({ url: '*://*.hero-sms.com/*' });
          ...
          async function waitForTabComplete(tabId, timeoutMs = 20000) {
            ...
            while (Date.now() - start < timeoutMs) {
              try {
                ...
                if (tab?.status === 'complete' && HERO_HOSTNAME_PATTERN.test(String(tab.url || ''))) {
                  return true;
                }
              } catch {
                return false;
          ...
          // 在页面上下文里跑的 fetch 函数 —— 一次性把所有需要的接口都打掉：
          //   1. deliverability                                      —— Top10 解码率
          //   2. handler_api.php getPrices(no country)               —— 全部国家「起价 + 物理库存」
          //   3. handler_api.php getCountries                        —— id → 中英名
          //   4. /api/v1/left-menu/service/<svc>/country/<id>/offers —— 每个 Top10 国家「实时各档位空闲数」
          //
          // 第 4 步是关键：用户要看的是「实际能拿到号的最低价」，而 getPrices 只给「理论起价」。
          // offers 接口按 operator + 价位档返回 freePriceOffers={price: count}，扫一遍就能拿到
          // 「现在最低能买到的价 + 总可买数 + 各档分布」。
          //
          // 注意：函数体内不能引用外部闭包变量，所有参数必须通过 `args` 传进去。
          async function fetchInPageContext(svc, ival, succ, key) {
            try {
              const baseUrl = 'https://hero-sms.com/stubs/handler_api.php';
              const delivUrl = `https://hero-sms.com/api/v1/stats/deliverability?service=${encodeURIComponent(svc)}&interval=${ival}&successCount=${encodeURIComponent(succ)}`;
              const pricesUrl = `${baseUrl}?api_key=${encodeURIComponent(key)}&action=getPrices&service=${encodeURIComponent(svc)}`;
              const countriesUrl = `${baseUrl}?api_key=${encodeURIComponent(key)}&action=getCountries`;
              const [d, p, c] = await Promise.all([
                fetch(delivUrl, { credentials: 'include' }),
                fetch(pricesUrl),
                fetch(countriesUrl),
              ]);
              if (d.status === 401 || d.status === 403) {
                return { error: 'NOT_LOGGED_IN', delivStatus: d.status };
              }
              if (!d.ok) return { error: `DELIV_HTTP_${d.status}` };
              if (!p.ok) return { error: `PRICES_HTTP_${p.status}` };
              if (!c.ok) return { error: `COUNTRIES_HTTP_${c.status}` };
              const [deliverability, prices, countries] = await Promise.all([d.json(), p.json(), c.json()]);

              // 从 deliverability 中提取 Top10 国家 ID，再并发拉每个国家的实时档位数据。
              // 这是 10 次额外请求，但都在同一个 tab 的页面上下文里，cookie 自动带，
              // 而且 Promise.all 并发 → 整体延迟约等于单次往返。
              const top10Ids = (deliverability?.data || [])
                .map((row) => Math.floor(Number(row?.country)))
                .filter((id) => Number.isFinite(id) && id > 0)
                .slice(0, 10);
              const offersByCountry = {};
              const offerResults = await Promise.all(top10Ids.map(async (id) => {
                try {
                  const r = await fetch(`https://hero-sms.com/api/v1/left-menu/service/${encodeURIComponent(svc)}/country/${id}/offers`, { credentials: 'include' });
              ...
              for (const item of offerResults) {
                if (item.ok && item.data) offersByCountry[item.id] = item.data;
              }

              return { ok: true, deliverability, prices, countries, offersByCountry };
            } catch (err) {
              return { error: `IN_PAGE_FETCH_ERROR: ${err?.message || err}` };
            }
          }

          let openedTabId = null;
          try {
            let heroTab = await findHeroTab();
            let needCleanup = false;
            if (!heroTab?.id) {
              // 没开 hero-sms.com 标签页，临时开一个；拉完不主动关，留给用户复用
              heroTab = await chrome.tabs.create({ url: TARGET_URL, active: false });
              openedTabId = heroTab?.id || null;
              needCleanup = false; // 不关，但记一下是不是我们开的
            }
            if (!heroTab?.id) {
              return { error: '无法打开 hero-sms.com 标签页。', code: 'NO_TAB' };
            }

            // 等页面加载完，确保 cookie 已经被注入到那个 tab
            await waitForTabComplete(heroTab.id, 25000);

            const scriptResults = await chrome.scripting.executeScript({
              target: { tabId: heroTab.id },
              world: 'MAIN',
              func: fetchInPageContext,
              args: [service, interval, successCount, apiKey],
            });
            ...
            if (!result || result.error) {
              ...
              if (code === 'NOT_LOGGED_IN') {
                return {
                  error: 'hero-sms.com 还没登录（即使页面已经打开，session cookie 还没生效）。请在弹出的标签页里完成登录后再点按钮。',
                  code: 'NOT_LOGGED_IN',
                  loginUrl: TARGET_URL,
                };
              }
              return {
                error: `页面上下文 fetch 失败：${code}`,
                code: 'IN_PAGE_FETCH_ERROR',
              };
            }
            ...
            if (!statsApi || typeof statsApi.enrichHeroSmsTopStats !== 'function') {
              return { error: 'HeroSMS 统计聚合模块未加载（PhoneSmsHeroSmsStats）。', code: 'MODULE_MISSING' };
            }

            const baseEnriched = statsApi.enrichHeroSmsTopStats({
              deliverability: result.deliverability,
              prices: result.prices,
              countries: result.countries,
              service,
            });
            // 关键：把每个 Top10 国家的「当前各档位空闲数」叠上去，让 row.cost 从「起价」
            // 变成「现在真正能买到的最低价」。如果某个国家 offer 接口失败，row.offerLookupFailed=true，
            // cost 回退到起价（startingCost 字段保留原值）。
            const enriched = typeof statsApi.mergeOffersIntoEnriched === 'function'
              ? statsApi.mergeOffersIntoEnriched(baseEnriched, result.offersByCountry || {})
              : baseEnriched;
            const recommended = statsApi.recommendHeroSmsTopStats(enriched);
            ...
            return {
              ...
              successCount,
              fetchedAt: Date.now(),
              tabReused: !openedTabId,
              enriched,
              recommended,
            };
          } catch (error) {
            return {
              error: `拉取 HeroSMS Top10 解码率失败：${error?.message || error}`,
              code: 'NETWORK_ERROR',
            };
          }
        }
