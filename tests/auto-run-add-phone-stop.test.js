...
test('auto-run controller retries NO_NUMBERS up to 10 times in the same round before giving up and continuing to next round', async () => {
  const events = {
    logs: [],
    broadcasts: [],
    accountRecords: [],
  ...
  const controller = api.createAutoRunController({
    ...
    runAutoSequenceFromStep: async () => {
      events.runCalls += 1;
      // Round 1: throw NO_NUMBERS on the first 11 attempts (1 initial + 10 retries cap),
      // then the 12th call is round 2 attempt 1 which should succeed.
      if (events.runCalls <= 11) {
  ...
  // Round 1 = 1 initial attempt + 10 same-round retries = 11 calls, all NO_NUMBERS.
  // Then round 2 attempt 1 succeeds = 1 more call. Total = 12.
  assert.equal(events.runCalls, 12, 'NO_NUMBERS should trigger 10 same-round retries before moving to next round');

  // Should NOT enter auto-run-controller's 'stopped' phase — we want to continue to next round.
  assert.equal(events.broadcasts.some(({ phase }) => phase === 'stopped'), false, 'NO_NUMBERS retries should not stop the entire auto-run');
  assert.equal(runtime.state.autoRunActive, false);
  assert.equal(runtime.state.autoRunSessionId, 0);

  // Round 1 ultimately failed (recorded once). Round 2 succeeded (no failed record for round 2).
  assert.equal(events.accountRecords.length, 1, 'only round 1 should record a failed account run');
  assert.equal(events.accountRecords[0].status, 'failed');
  assert.match(events.accountRecords[0].reason, /NO_NUMBERS/i);

  // There should be retry-progress logs like "自动重试第 N/10 次"
  const retryLogs = events.logs.filter(({ message }) => /自动重试第 \d+\/10 次/.test(message));
  assert.equal(retryLogs.length, 10, 'should log each of the 10 same-round retries');
  assert.ok(events.logs.some(({ message }) => /连续 10 次仍无可用号码，本轮放弃/.test(message)), 'should log a give-up message after hitting the cap');
  assert.ok(events.logs.some(({ message }) => /自动流程将继续下一轮/.test(message)), 'should log that flow will continue to the next round');
});
