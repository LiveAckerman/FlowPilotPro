...
test('inspectSignupEntryState treats chatgpt auth login as reusable identity entry', () => {
  const api = new Function(`
const switchButton = {
...
${extractFunction('isChatgptAuthLoginEntryUrl')}
${extractFunction('findSignupUseEmailTrigger')}
${extractFunction('findSignupUsePhoneTrigger')}
${extractFunction('findSignupEntryTrigger')}
${extractFunction('inspectSignupEntryState')}
...
return {
  run() {
    ...
    return {
      ...
      switchText: getActionText(snapshot.switchToPhoneTrigger),
      url: snapshot.url,
    };
  },
};
...
test('ensureSignupPhoneEntryReady switches from chatgpt auth login combined page to phone entry', async () => {
  const api = new Function(`
const logs = [];
const clicks = [];
let phase = 'combined';
...
const document = {
  querySelector(selector) {
    if (selector === SIGNUP_EMAIL_INPUT_SELECTOR) {
      return phase === 'combined' ? emailInput : null;
    }
    if (selector === SIGNUP_PHONE_INPUT_SELECTOR) {
      return phase === 'phone' ? phoneInput : null;
    }
  ...
  querySelectorAll(selector) {
    if (selector === 'button, a, [role="button"], [role="link"]') {
      return phase === 'combined' ? [switchButton] : [];
    }
    if (selector === 'a, button, [role="button"], [role="link"]') {
      return [];
    }
    if (selector === 'input') {
      if (phase === 'combined') return [emailInput];
      if (phase === 'phone') return [phoneInput];
      return [];
    }
    return [];
...
function getPageTextSnapshot() {
  return phase === 'combined' ? '登录或注册 使用电话号码继续 或 继续 抢先试用' : '';
}
