(function attachBackgroundPhoneVerification(root, factory) {
  ...
  function createPhoneVerificationHelpers(deps = {}) {
    ...
    function shouldAutoResendPhoneCode(state = {}) {
      const parsed = Math.floor(Number(state?.verificationResendCount));
    ...
    function collectHeroSmsTopCountryEntries(payload, countryId, entries = []) {
    ...
    async function fetchHeroSmsPricePayloads(config, countryConfig, options = {}) {
      ...
      const actions = Array.isArray(options.actions) && options.actions.length
        ...
        : (
          shouldUseHeroSmsExpandedPriceLookup(options.state || {})
            ? ['getPricesExtended', 'getPrices', 'getPricesForVerification', 'getPricesVerification', 'getTopCountriesByService']
            : ['getPrices']
        );
      ...
      for (const action of actions) {
        try {
          ...
          if (action === 'getTopCountriesByService') {
            payloads.push(...collectHeroSmsTopCountryEntries(payload, countryConfig.id, []));
          } else {
    ...
    function isPhoneResendBannedNumberError(error) {
      ...
      return /无法向此电话号码发送(?:短信|(?:文本)?消息)|无法向此手机号发送(?:短信|(?:文本)?消息)|无法发送(?:短信|(?:文本)?消息)到此电话号码|无法发送(?:短信|(?:文本)?消息)到此手机号|can(?:not|'t)\s+send\s+(?:an?\s+)?(?:sms|text(?:\s+message)?)\s+to\s+(?:this|that)\s+(?:phone\s+)?number|unable\s+to\s+send\s+(?:an?\s+)?(?:sms|text(?:\s+message)?)\s+to\s+(?:this|that)\s+(?:phone\s+)?number/i.test(message);
    }
      ...
      return withPhoneVerificationLogContext({ step: visibleStep, stepKey }, async () => {
        ...
        for (let windowIndex = 1; windowIndex <= timeoutWindows; windowIndex += 1) {
          ...
          try {
            ...
            if (windowIndex < timeoutWindows) {
              if (!shouldAutoResendPhoneCode(state)) {
                await addLog(
                  `步骤 ${visibleStep}：${normalizedActivation.phoneNumber} 在 ${waitSeconds} 秒内未收到短信；验证码重发次数为 0，将继续等待但不请求重发。`,
                  'warn',
                  { step: visibleStep, stepKey }
                );
                continue;
