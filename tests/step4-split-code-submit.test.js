...
test('prepareSignupVerificationFlow stops phone signup when password page is login route without visible error', async () => {
  const api = new Function(`
const logs = [];
const clicks = [];
let now = 0;
...
function findResendVerificationCodeTrigger() { return null; }
function isEmailVerificationPage() { return false; }
function getPageTextSnapshot() { return 'Input password Phone number Forgot password Continue'; }
function getVerificationCodeTarget() { return null; }
function is405MethodNotAllowedPage() { return false; }
async function recoverCurrentAuthRetryPage() {}
function createSignupUserAlreadyExistsError() { return new Error('user already exists'); }
...
const document = {
  ...
  title: '',
  body: {
    textContent: 'Input password Phone number Forgot password Continue',
    innerText: 'Input password Phone number Forgot password Continue',
  },
  querySelector() {
    return null;
  },
...
${extractFunction('isSignupLoginPasswordPageUrl')}
${extractFunction('isSignupVerificationPageInteractiveReady')}
${extractFunction('isVerificationPageStillVisible')}
${extractFunction('isSignupProfilePageUrl')}
${extractFunction('isLikelyLoggedInChatgptHomeUrl')}
...
`)();
  ...
  assert.equal(result.threw, true);
  assert.match(result.error, /SIGNUP_PHONE_PASSWORD_MISMATCH::/);
  assert.match(result.error, /登录密码页/);
  assert.equal(result.clicks.length, 0);
  assert.equal(result.logs.some(({ message }) => /登录密码页/.test(message)), true);
});
