...
async function handlePollEmail(step, payload) {
  ...
  const getSkipSnippet = (text) => String(text || '').replace(/\s+/g, ' ').trim().slice(0, 160) || '空';
  const getTimeLabel = (timestamp) => timestamp
  ...
  initialItems = Array.isArray(initialMailbox.items) ? initialMailbox.items : [];
  if (!initialMailbox.ready || initialItems.length === 0) {
    ...
    if (!refreshedMailbox.ready) {
      log(`步骤 ${step}：2925 邮箱列表暂未确认加载完成，将继续刷新等待新验证码邮件。`, 'warn');
    }
    initialItems = Array.isArray(refreshedMailbox.items) ? refreshedMailbox.items : [];
  }

  if (initialItems.length === 0) {
    log(`步骤 ${step}：当前 2925 邮箱列表为空，将继续刷新等待新验证码邮件。`, 'info');
  } else {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    if (!mailbox.ready) {
      log(`步骤 ${step}：2925 邮箱列表暂未确认加载完成，本轮继续刷新等待。`, 'warn');
    }
    const items = mailbox.ready && Array.isArray(mailbox.items) ? mailbox.items : [];
    if (items.length > 0) {
      for (let index = 0; index < items.length; index += 1) {
        ...
        const mailLabel = `第 ${index + 1}/${items.length} 封`;

        if (filterAfterMinute && (!itemMinute || itemMinute < filterAfterMinute)) {
          log(`步骤 ${step}：跳过 2925 ${mailLabel}：时间不在本轮窗口内（邮件时间：${getTimeLabel(itemTimestamp)}，窗口起点：${getTimeLabel(filterAfterMinute)}）。预览：${getSkipSnippet(previewText)}`, 'info');
          continue;
        }

        if (!matchesMailFilters(previewText, senderFilters, subjectFilters)) {
          log(`步骤 ${step}：跳过 2925 ${mailLabel}：预览未命中发件人/标题关键字。预览：${getSkipSnippet(previewText)}`, 'info');
          continue;
        }
        ...
        if (mail2925MatchTargetEmail && previewTargetState.hasExplicitEmail && !previewTargetState.matches) {
          log(`步骤 ${step}：2925 ${mailLabel} 预览中的邮箱与当前注册邮箱不一致，将打开正文继续确认（目标：${targetEmail || '空'}）。预览：${getSkipSnippet(previewText)}`, 'info');
        }
        ...
        if (mail2925MatchTargetEmail && openedTargetState.hasExplicitEmail && !openedTargetState.matches) {
          log(`步骤 ${step}：跳过 2925 ${mailLabel}：正文中的目标邮箱与当前注册邮箱不一致（目标：${targetEmail || '空'}）。正文：${getSkipSnippet(openedText)}`, 'info');
          continue;
        }
        ...
        if (!candidateCode) {
          log(`步骤 ${step}：跳过 2925 ${mailLabel}：已打开邮件但未提取到 6 位验证码。正文：${getSkipSnippet(openedText)}`, 'info');
          continue;
        }
