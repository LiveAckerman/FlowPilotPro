(function attachSignupFlowHelpers(root, factory) {
  ...
  function createSignupFlowHelpers(deps = {}) {
    ...
    async function finalizeSignupPasswordSubmitInTab(tabId, password = '', step = 3, completionPayload = {}) {
      ...
      try {
        result = await sendToContentScriptResilient('openai-auth', {
          ...
          payload: {
            ...
            accountIdentifierType: completionPayload?.accountIdentifierType || '',
            accountIdentifier: completionPayload?.accountIdentifier || '',
            phoneNumber: completionPayload?.phoneNumber || '',
          },
        }, {
          timeoutMs: 30000,
          retryDelayMs: 700,
