(function attachBackgroundStep2(root, factory) {
  ...
  function createStep2Executor(deps = {}) {
    ...
    async function executeSignupPhoneEntry(state) {
      ...
      // 手机号注册落到 /log-in/password 意味着 OpenAI 已识别该号码归属现有账号，需要立即换号重跑。
      // 这里在 submit-signup-email 阶段就拦截，避免后续节点用同一脏号继续。
      const landingUrl = String(landingResult?.url || '');
      if (
        landingResult?.state === 'password_page'
        && /\/log-in\/password(?:[/?#]|$)/i.test(landingUrl)
      ) {
        if (activation && typeof phoneVerificationHelpers?.cancelSignupPhoneActivation === 'function') {
          await phoneVerificationHelpers.cancelSignupPhoneActivation(state, activation).catch(() => {});
        }
        throw new Error(
          `PHONE_RESEND_BANNED_NUMBER::步骤 2：当前注册手机号 ${phoneNumber} 已被 OpenAI 识别为已存在账号（落入登录密码页 ${landingUrl}），需要更换手机号重新开始当前轮。`
        );
      }
