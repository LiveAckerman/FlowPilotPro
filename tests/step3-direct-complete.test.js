...
test('step 3 stops phone signup when password page is actually login route', async () => {
  const api = new Function(`
const events = [];
const location = {
...
async function humanPause() { events.push('pause'); }
function reportComplete() { events.push('complete'); }
...
return {
  async run() {
    try {
      await step3_fillEmailPassword({
        accountIdentifierType: 'phone',
        phoneNumber: '5553984850130',
        password: 'Secret123!',
      });
      return { threw: false, events, passwordValue: passwordInput.value };
    } catch (error) {
      return { threw: true, error: error.message, events, passwordValue: passwordInput.value };
    }
  },
};
`)();
  ...
  assert.equal(result.passwordValue, '');
});
