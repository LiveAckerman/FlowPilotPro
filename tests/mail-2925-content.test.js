...
test('handlePollEmail keeps polling when the 2925 inbox is initially empty', async () => {
  const bundle = [
    extractFunction('normalizeMinuteTimestamp'),
    extractFunction('handlePollEmail'),
  ].join('\n');
...
const seenCodes = new Set();
const readAndDeleteCalls = [];
const newMail = { id: 'new', text: 'OpenAI verification code 246810' };
...
test('handlePollEmail opens 2925 mail when only the preview target email mismatches', async () => {
  const bundle = [
    extractFunction('extractEmails'),
...
const mail = {
  id: 'mail-1',
  text: 'OpenAI temporary code noreply@tm.openai.com Bounce0=3738003=20583=wkka3c@duck.com',
  body: 'Your OpenAI verification code is 135790 for claim-could-deacon@duck.com',
};
...
function matchesMailFilters(text) {
  return /openai|verification|code/i.test(String(text || ''));
}
...
`)();
  ...
  const result = await api.handlePollEmail(8, {
    ...
    maxAttempts: 1,
    intervalMs: 1,
    targetEmail: 'claim-could-deacon@duck.com',
