...
test('phone verification helper accepts HeroSMS price field when building buy maxPrice', async () => {
  const requests = [];
  const helpers = api.createPhoneVerificationHelpers({
    addLog: async () => {},
    ensureStep8SignupPageReady: async () => {},
    fetchImpl: async (url) => {
      ...
      if (action === 'getNumber') {
        return {
          ok: true,
          text: async () => 'ACCESS_NUMBER:brazil-123:5511999912345',
        };
      }
  ...
  assert.equal(activation.activationId, 'brazil-123');
  assert.equal(activation.countryId, 73);
  const buyRequest = requests.find((request) => request.searchParams.get('action') === 'getNumber');
  assert.equal(buyRequest.searchParams.get('country'), '73');
  assert.equal(buyRequest.searchParams.get('maxPrice'), '0.0277');
  assert.equal(buyRequest.searchParams.get('fixedPrice'), 'true');
});

test('phone verification helper uses expanded HeroSMS verification price tiers before giving up', async () => {
  const requests = [];
  const helpers = api.createPhoneVerificationHelpers({
    addLog: async () => {},
    ensureStep8SignupPageReady: async () => {},
    fetchImpl: async (url) => {
      ...
      if (action === 'getPricesForVerification' || action === 'getPricesVerification') {
        return {
          ok: true,
          text: async () => JSON.stringify({
            73: {
      ...
      if ((action === 'getNumber' || action === 'getNumberV2') && maxPrice === '0.025') {
        return {
          ok: true,
          text: async () => 'NO_NUMBERS',
        };
      }
      if (action === 'getNumber' && maxPrice === '0.0277') {
        return {
          ok: true,
          text: async () => 'ACCESS_NUMBER:brazil-277:5511999912345',
        };
      }
  ...
  assert.equal(activation.activationId, 'brazil-277');
  assert.deepStrictEqual(
    requests
      .filter((request) => ['getNumber', 'getNumberV2'].includes(request.searchParams.get('action')))
      .map((request) => `${request.searchParams.get('action')}:${request.searchParams.get('maxPrice')}`),
    ['getNumber:0.025', 'getNumberV2:0.025', 'getNumber:0.0277']
  );
});
...
test('signup phone helper does not resend SMS when verificationResendCount is zero', async () => {
  const requests = [];
  const messages = [];
  let currentState = {
    heroSmsApiKey: 'demo-key',
  ...
  const helpers = api.createPhoneVerificationHelpers({
    addLog: async (message) => messages.push(String(message || '')),
    ensureStep8SignupPageReady: async () => {},
    fetchImpl: async (url) => {
      const parsedUrl = new URL(url);
    ...
    getState: async () => currentState,
    sendToContentScriptResilient: async () => {
      throw new Error('page resend should not be called');
    },
    setState: async (updates) => {
      currentState = { ...currentState, ...updates };
    },
  ...
  await assert.rejects(
    () => helpers.waitForSignupPhoneCode(currentState, currentState.signupPhoneActivation),
    /等待手机验证码超时/
  );

  assert.ok(requests.length >= 2);
  assert.ok(requests.every((action) => action === 'getStatus'));
  assert.ok(!requests.includes('setStatus'));
  assert.ok(messages.some((message) => /验证码重发次数为 0/.test(message)));
});
...
test('phone verification helper infers major HeroSMS countries from phone prefixes', async () => {
  const cases = [
    ...
    { phoneNumber: '5511999912345', countryId: 73, countryLabel: 'Brazil' },
