...
test('step 5 navigation reporter completes on pagehide so background can validate new page', () => {
  const api = new Function(`
const events = [];
const listeners = new Map();
const location = {
...
`)();
  ...
  assert.deepStrictEqual(result.completions[0], {
  ...
  assert.equal(result.events.some((entry) => entry.type === 'log' && /event=pagehide/.test(entry.message)), true);
});
