...
const SIGNUP_ENTRY_TRIGGER_PATTERN = /免费注册|立即注册|注册|抢先试用|無料でサインアップ|サインアップ|新規登録|登録する|登録|アカウントを作成|アカウント作成|sign\s*up|register|create\s*account|create\s+account/i;
...
function isChatgptAuthLoginEntryUrl(rawUrl = location.href) {
  ...
  try {
    const parsed = new URL(value, location?.origin || 'https://chatgpt.com');
    const host = String(parsed.hostname || '').toLowerCase();
    return ['chatgpt.com', 'www.chatgpt.com'].includes(host)
      && /^\/auth\/login(?:[/?#]|$)/i.test(parsed.pathname || '');
  } catch {
    return /^https:\/\/(?:www\.)?chatgpt\.com\/auth\/login(?:[/?#]|$)/i.test(value)
      || /^\/auth\/login(?:[/?#]|$)/i.test(value);
  }
}
...
function findSignupUseEmailTrigger(scope) {
...
function getActiveSignupDialog() {
...
function findSignupUsePhoneTrigger(scope) {
  // scope 用来把搜索限定在某个 modal 内部，避免被页面或扩展注入的 stale 元素误导。
  const root = scope || document;
  if (!root || typeof root.querySelectorAll !== 'function') return null;
  const candidates = root.querySelectorAll('button, a, [role="button"], [role="link"]');
  return Array.from(candidates).find((el) => {
...
function inspectSignupEntryState() {
  ...
  // chatgpt.com 主页登录/注册流程的本质：弹一个 [role="dialog"] modal。所有真实的
  // input/button 都在 modal 内部。直接对 document 全局查询会被以下两种情况坑：
  //   1. 主页或浏览器扩展注入的 stale email input 干扰 → 误判 email_entry
  //   2. 上一轮关掉的 modal 残留 DOM 节点 → 误判 phone_entry
  // 一旦发现有可见的 modal，就**只在 modal 内部**判定状态。
  // 用 typeof 守卫以兼容测试 harness 未注入 getActiveSignupDialog 的场景。
  const activeSignupDialog = typeof getActiveSignupDialog === 'function'
    ? getActiveSignupDialog()
    : null;
  const inputSearchRoot = activeSignupDialog || (typeof document !== 'undefined' ? document : null);

  // 优先识别 OpenAI 当前手机号注册页/弹窗用的手机号 input。这一段必须放在 email_entry
  // 之前，否则页面同时存在邮箱 input + 手机号 input 时（用户已经手动切到手机号模式但
  // email 节点仍在 DOM 里），会被早返回为 email_entry，后续再去点「切换到手机号」就会
  // 因为已经在手机号模式找不到切换按钮而超时。这里**不强制 isVisibleElement**，
  // 因为手机号 input 在 modal 弹出初期常是 width=0/height=0/clip-path 隐藏但 DOM 存在。
  // 已知 OpenAI 用过的 marker（按优先级）：
  //   1. id="phoneNumberInput" —— chatgpt.com 主页登录/注册弹窗
  //   2. name="phoneNumber" / name="phone_number" —— auth.openai.com 老版页面
  //   3. autocomplete="tel" —— 通用兜底
  let phoneNumberInputByMarker = null;
  let phoneInputDetectedBy = '';
  if (inputSearchRoot && typeof inputSearchRoot.querySelector === 'function') {
    // 注意：在 dialog 内部用 querySelector('#phoneNumberInput')，避免 getElementById
    // 找到 dialog 外残留的同 id 元素。
    const byId = inputSearchRoot.querySelector('#phoneNumberInput');
    if (byId) {
      phoneNumberInputByMarker = byId;
      phoneInputDetectedBy = activeSignupDialog ? 'phoneNumberInput-id-in-dialog' : 'phoneNumberInput-id';
    }
    if (!phoneNumberInputByMarker) {
      const byName = inputSearchRoot.querySelector('input[name="phoneNumber"], input[name="phone_number"], input[name="phoneNumberInput"]');
      if (byName) {
        phoneNumberInputByMarker = byName;
        phoneInputDetectedBy = activeSignupDialog ? 'phoneNumber-name-in-dialog' : 'phoneNumber-name';
      }
    }
    if (!phoneNumberInputByMarker) {
      const byAutocomplete = inputSearchRoot.querySelector('input[autocomplete="tel"], input[autocomplete="tel-national"]');
      if (byAutocomplete) {
        phoneNumberInputByMarker = byAutocomplete;
        phoneInputDetectedBy = activeSignupDialog ? 'autocomplete-tel-in-dialog' : 'autocomplete-tel';
      }
    }
  }
  if (phoneNumberInputByMarker) {
    return {
      state: 'phone_entry',
      phoneInput: phoneNumberInputByMarker,
      switchToEmailTrigger: findSignupUseEmailTrigger(activeSignupDialog),
      detectedBy: phoneInputDetectedBy,
      dialogOpen: Boolean(activeSignupDialog),
      url: location.href,
    };
  }

  // 关键中间态：OpenAI 的 modal 切到 phone 模式后，switch 按钮文本会立刻从
  // 「使用电话号码继续」变成「使用电子邮箱继续」，但真正的 phoneNumberInput 渲染
  // 有几秒延迟（要做 IP 地理定位拿国家代码、加载国家列表）。这个空窗期 DOM 里
  // 仍然残留 email input，会被误判为 email_entry，loop 又去 click「使用电子邮箱
  // 继续」当成 switch-to-phone（其实方向反了），把 modal 切回 email 模式，无限
  // 循环。
  //
  // 修复：只要 dialog 内出现了 switch-to-email 按钮（说明已经在 phone 模式），
  // 即使 phone input 还没渲染，也返回 phone_entry_pending 状态，让上层只等不点。
  if (activeSignupDialog) {
    const switchToEmailInDialog = findSignupUseEmailTrigger(activeSignupDialog);
    if (switchToEmailInDialog) {
      return {
        state: 'phone_entry_pending',
        switchToEmailTrigger: switchToEmailInDialog,
        dialogOpen: true,
        url: location.href,
      };
    }
  ...
  // dialog 内部找邮箱 input；若没有 dialog 才回落到全局搜索（兼容 auth.openai.com 这种
  // 没有 modal 的传统认证页）。
  const emailInput = activeSignupDialog
    ? activeSignupDialog.querySelector('input[type="email"], input[id="email"], input[name="email"], input[autocomplete="email"]')
    : getSignupEmailInput();
  if (emailInput) {
    return {
      state: 'email_entry',
      ...
      switchToPhoneTrigger: findSignupUsePhoneTrigger(activeSignupDialog),
      dialogOpen: Boolean(activeSignupDialog),
      url: location.href,
    };
  }

  const phoneInput = activeSignupDialog
    ? activeSignupDialog.querySelector('input[type="tel"]')
    : getSignupPhoneInput();
  if (phoneInput) {
    return {
      state: 'phone_entry',
  ...
  if (typeof isChatgptAuthLoginEntryUrl === 'function' && isChatgptAuthLoginEntryUrl()) {
    const switchToPhoneTrigger = findSignupUsePhoneTrigger();
    const switchToEmailTrigger = findSignupUseEmailTrigger();
    if (emailInput || switchToPhoneTrigger) {
      return {
        state: 'email_entry',
        emailInput,
        continueButton: getSignupEmailContinueButton({ allowDisabled: true }),
    ...
    if (phoneInput || switchToEmailTrigger) {
      return {
        state: 'phone_entry',
        phoneInput,
...
function getSignupEntryStateSummary(snapshot = inspectSignupEntryState()) {
  const phoneNumberInputById = (typeof document !== 'undefined'
    && typeof document.getElementById === 'function')
    ? document.getElementById('phoneNumberInput')
    : null;
  const activeDialog = typeof getActiveSignupDialog === 'function' ? getActiveSignupDialog() : null;
  const summary = {
    state: snapshot?.state || 'unknown',
    url: snapshot?.url || location.href,
    hasEmailInput: Boolean(snapshot?.emailInput || getSignupEmailInput()),
    hasPhoneInput: Boolean(snapshot?.phoneInput || getSignupPhoneInput() || phoneNumberInputById),
    hasPhoneNumberInputById: Boolean(phoneNumberInputById),
    hasPasswordInput: Boolean(snapshot?.passwordInput || getSignupPasswordInput()),
    hasActiveDialog: Boolean(activeDialog),
    activeDialogHasPhoneInput: activeDialog ? Boolean(activeDialog.querySelector('#phoneNumberInput, input[type="tel"]')) : false,
    activeDialogHasEmailInput: activeDialog ? Boolean(activeDialog.querySelector('input[type="email"], input[id="email"]')) : false,
    activeDialogHasSwitchToPhone: activeDialog ? Boolean(findSignupUsePhoneTrigger(activeDialog)) : false,
    activeDialogHasSwitchToEmail: activeDialog ? Boolean(findSignupUseEmailTrigger(activeDialog)) : false,
  };
  if (snapshot?.detectedBy) {
    summary.detectedBy = snapshot.detectedBy;
  }
  if (snapshot?.dialogOpen !== undefined) {
    summary.dialogOpen = snapshot.dialogOpen;
  }
...
function getSignupEntryDiagnostics() {
  ...
  return {
    ...
    hasPhoneInput: Boolean(
      getSignupPhoneInput()
      || (typeof document.getElementById === 'function' && document.getElementById('phoneNumberInput'))
      || (typeof document.querySelector === 'function' && document.querySelector('input[name="phoneNumber"], input[name="phone_number"], input[name="phoneNumberInput"], input[autocomplete="tel"], input[autocomplete="tel-national"]'))
    ),
    hasPhoneNumberInputById: Boolean(typeof document.getElementById === 'function' && document.getElementById('phoneNumberInput')),
    hasPhoneInputByName: Boolean(typeof document.querySelector === 'function' && document.querySelector('input[name="phoneNumber"], input[name="phone_number"], input[name="phoneNumberInput"]')),
    hasPhoneInputByAutocomplete: Boolean(typeof document.querySelector === 'function' && document.querySelector('input[autocomplete="tel"], input[autocomplete="tel-national"]')),
...
async function waitForSignupEntryState(options = {}) {
  ...
  while (Date.now() - start < timeout) {
    ...
    if (snapshot.state === 'email_entry') {
      if (snapshot.emailInput || !autoOpenEntry) {
        return snapshot;
      }

      if (logDiagnostics && !slowSnapshotLogged && Date.now() - start >= 5000) {
        slowSnapshotLogged = true;
        log(`步骤 ${step}：等待邮箱输入框超过 5 秒，页面诊断快照：${JSON.stringify(getSignupEntryDiagnostics())}`, 'warn');
      }
...
async function waitForSignupPhoneEntryState(options = {}) {
  ...
  while (true) {
    ...
    const directSwitchToPhone = findSignupUsePhoneTrigger();
    if (directSwitchToPhone && Date.now() - lastSwitchToPhoneAt >= 1500) {
      lastSwitchToPhoneAt = Date.now();
      log(`步骤 ${step}：检测到手机号注册入口，正在切换到手机号输入模式...`);
      await humanPause(350, 900);
      await performOperationWithDelay({ stepKey: 'signup-phone-entry', kind: 'click', label: 'switch-to-signup-phone' }, async () => {
        simulateClick(directSwitchToPhone);
      });
      await sleep(250);
      continue;
    }
...
async function step3_fillEmailPassword(payload) {
  ...
  if (accountIdentifierType === 'phone' && isSignupLoginPasswordPageUrl(snapshot.url || location.href)) {
    throw createSignupPhonePasswordMismatchError('当前手机号已进入登录密码页，可能已关联现有账号。');
  }
...
const SIGNUP_UNSUPPORTED_COUNTRY_ERROR_PREFIX = 'SIGNUP_UNSUPPORTED_COUNTRY::';
// OpenAI 在不可用区域会返回 unsupported_country：英文页是
// "OpenAI's services are not available in your country."；中文页是
// "OpenAI 服务在你所在的国家/地区不可用。"；错误代码 unsupported_country。
const SIGNUP_UNSUPPORTED_COUNTRY_PATTERN = /unsupported_country|OpenAI\s*(?:'s)?\s*服务在你所在的(?:国家|地区)(?:\s*[\\\/、，,]\s*(?:国家|地区))?(?:暂时)?不可用|OpenAI(?:'s|s)?\s*(?:services?|service)(?:\s+(?:is|are))?\s+not\s+available\s+in\s+your\s+(?:country|region|location)/i;
...
function detectSignupUnsupportedCountryError() {
  // 同时看 document.title / 错误代码区域 / 全页正文，避免 OpenAI 改文案后漏判。
  const title = String(document?.title || '').replace(/\s+/g, ' ').trim();
  const pageText = String(getPageTextSnapshot?.() || '').replace(/\s+/g, ' ').trim();
  const combined = `${title} ${pageText}`.trim();
  if (!combined) return '';
  if (!SIGNUP_UNSUPPORTED_COUNTRY_PATTERN.test(combined)) {
    return '';
  }
  // 优先提取「错误代码：unsupported_country」附近一段，方便日志诊断。
  const conciseMatch = combined.match(
    /OpenAI[^。.!?]{0,80}(?:不可用|not\s+available[^。.!?]{0,40})[。.!?]?|错误代码\s*[:：]\s*unsupported_country|unsupported_country/i
  );
  return String(conciseMatch?.[0] || '该地区不支持 OpenAI 服务（unsupported_country）').trim();
}

function createSignupUnsupportedCountryError(detailText = '') {
  const detail = String(detailText || '').replace(/\s+/g, ' ').trim()
    || '页面提示：OpenAI 服务在你所在的国家/地区不可用（unsupported_country）。';
  return new Error(
    `${SIGNUP_UNSUPPORTED_COUNTRY_ERROR_PREFIX}步骤 5：当前 IP 所在区域被 OpenAI 拒绝（unsupported_country）。${detail}请检查 VPN 是否已挂载到 OpenAI 支持的区域（建议美国干净住宅 IP），切换后再重试。`
  );
}
...
function isSignupLoginPasswordPageUrl(rawUrl = location.href) {
  const value = String(rawUrl || '').trim();
  if (!value) {
    return false;
  }
  ...
  try {
    const parsed = new URL(value, location?.origin || 'https://auth.openai.com');
    return /\/log-in\/password(?:[/?#]|$)/i.test(parsed.pathname || '');
  } catch {
    return /\/log-in\/password(?:[/?#]|$)/i.test(value);
  }
}
...
function getAuthTimeoutErrorPageState(options = {}) {
  ...
  // OpenAI 在不同时机会返回 email_in_use / email_already_in_use；
  // 中文版本则是「该邮箱地址已有关联账户」「该邮箱已被使用 / 已被绑定」。
  // 这三类都要识别，否则 bind-email/fetch-bind-email-code 的换号链路会漏触发。
  const emailInUseBlocked = /email(?:_already)?_in_use|该邮箱(?:地址)?(?:已有关联(?:账户)?|已被(?:使用|绑定|占用))|email\s+address.*already\s+(?:in\s+use|associated)/i.test(text);
...
async function prepareSignupVerificationFlow(payload = {}, timeout = 30000) {
  ...
  const isPhoneSignupFinalize = accountIdentifierType === 'phone' || Boolean(String(payload?.phoneNumber || '').trim());
  ...
  while (Date.now() - start < timeout && recoveryRound < maxRecoveryRounds) {
    ...
    if (snapshot.state === 'password') {
      ...
      if (isPhoneSignupFinalize && isSignupLoginPasswordPageUrl(snapshot.url || location.href)) {
        log(`${prepareLogLabel}：手机号注册落到登录密码页，当前号码可能已关联现有账号，将重新开始当前轮。`, 'warn');
        throw createSignupPhonePasswordMismatchError('当前手机号已进入登录密码页，可能已关联现有账号。');
      }
...
async function waitForVerificationSubmitOutcome(step, timeout, options = {}) {
  ...
  while (Date.now() - start < resolvedTimeout) {
    ...
    // step 8 = add-email；step 9 = bind-email；step 10 = fetch-bind-email-code。
    // 这三个步骤都可能在 /email-verification 收到 email_already_in_use，统一抛 STEP8_EMAIL_IN_USE。
    if ((step === 8 || step === 9 || step === 10) && retryState?.emailInUseBlocked) {
      throw createStep8EmailInUseError();
    }
    if ((step === 8 || step === 9 || step === 10) && retryState?.maxCheckAttemptsBlocked) {
...
async function submitAddEmailAndContinue(payload = {}) {
  ...
  if (
    outcome.errorText
    && (
      SIGNUP_EMAIL_EXISTS_PATTERN.test(outcome.errorText)
      || /email(?:_already)?_in_use|该邮箱(?:地址)?(?:已有关联(?:账户)?|已被(?:使用|绑定|占用))|email\s+address.*already\s+(?:in\s+use|associated)/i.test(outcome.errorText)
    )
  ) {
    throw createStep8EmailInUseError();
  }
...
async function step5_fillNameBirthday(payload) {
  ...
  // 区域受限拦截：在动手填资料前先看一眼页面是不是「OpenAI 服务在你所在的国家/地区不可用」。
  // 命中就抛 SIGNUP_UNSUPPORTED_COUNTRY，后台会停整轮自动运行并提示用户检查 VPN。
  // 用 typeof 守卫以兼容未注入这两个 helper 的测试 harness。
  if (
    typeof detectSignupUnsupportedCountryError === 'function'
    && typeof createSignupUnsupportedCountryError === 'function'
  ) {
    const unsupportedCountryDetail = detectSignupUnsupportedCountryError();
    if (unsupportedCountryDetail) {
      throw createSignupUnsupportedCountryError(unsupportedCountryDetail);
    }
  }
  const performOperationWithDelay = typeof getOperationDelayRunner === 'function'
    ? getOperationDelayRunner()
