...
const rowHeroSmsTopStats = document.getElementById('row-hero-sms-top-stats');
const btnHeroSmsTopStats = document.getElementById('btn-hero-sms-top-stats');
const btnHeroSmsTopStatsOpenSite = document.getElementById('btn-hero-sms-top-stats-open-site');
const heroSmsTopStatsHint = document.getElementById('hero-sms-top-stats-hint');
const heroSmsTopStatsResult = document.getElementById('hero-sms-top-stats-result');
...
const HERO_SMS_SUPPORTED_COUNTRY_ITEMS = Object.freeze([
  ...
  { id: 81, chn: "尼泊爾", eng: "Nepal" },
  { id: 82, chn: "比利時", eng: "Belgium" },
  { id: 83, chn: "保加利亞", eng: "Bulgaria" },
  { id: 84, chn: "匈牙利", eng: "Hungary" },
  { id: 85, chn: "摩爾多瓦", eng: "Moldova" },
  ...
  { id: 114, chn: "毛里塔尼亞", eng: "Mauritania" },
  ...
  { id: 152, chn: "布基納法索", eng: "Burkina Faso" },
...
function updatePhoneVerificationSettingsUI() {
  ...
  if (rowHeroSmsTopStats) rowHeroSmsTopStats.style.display = showSettings && heroProvider ? '' : 'none';
...
// 「打开 hero-sms.com」辅助按钮 —— 让用户先在浏览器里登录，session cookie 写入后
// 主按钮的 fetch(credentials:'include') 就能拉到 Top10 解码率统计。
btnHeroSmsTopStatsOpenSite?.addEventListener('click', async () => {
  try {
    await chrome.tabs.create({ url: 'https://hero-sms.com/cn/statistics/top10?service=dr', active: true });
  } catch (err) {
    if (heroSmsTopStatsHint) {
      heroSmsTopStatsHint.textContent = `打开标签页失败：${err?.message || err}`;
      heroSmsTopStatsHint.style.color = '#c00';
    }
  }
});

// 「获取热门便宜 HeroSMS 号码」主按钮：
// 1. 发消息给 background → 拉 deliverability + getPrices + getCountries 三个接口
// 2. background 合并/排序后回传 enriched 数组
// 3. 这里渲染成两个表：Top10（按页面默认成功率排）+ 推荐（过滤掉物理库存=0 + 重排）
btnHeroSmsTopStats?.addEventListener('click', async () => {
  if (!btnHeroSmsTopStats || !heroSmsTopStatsResult) return;

  const originalLabel = btnHeroSmsTopStats.textContent;
  btnHeroSmsTopStats.disabled = true;
  btnHeroSmsTopStats.textContent = '拉取中...';
  if (heroSmsTopStatsHint) {
    heroSmsTopStatsHint.textContent = '正在请求 HeroSMS Top10 解码率 + 价格 + 国家映射...';
    heroSmsTopStatsHint.style.color = '#888';
  }
  heroSmsTopStatsResult.style.display = 'none';
  heroSmsTopStatsResult.innerHTML = '';

  try {
    const response = await chrome.runtime.sendMessage({
      type: 'FETCH_HERO_SMS_TOP_STATS',
      source: 'sidepanel',
      payload: { service: 'dr', interval: 12, successCount: 'medium' },
    });
    ...
    if (!response || response.error) {
      ...
      if (heroSmsTopStatsHint) {
        ...
        if (code === 'NOT_LOGGED_IN') {
          heroSmsTopStatsHint.innerHTML = `${msg} <a href="#" id="hero-sms-top-stats-login-link" style="color:#06f;">点这里打开登录页</a>`;
          const link = document.getElementById('hero-sms-top-stats-login-link');
          link?.addEventListener('click', (ev) => {
            ev.preventDefault();
            chrome.tabs.create({ url: response.loginUrl || 'https://hero-sms.com/cn/statistics/top10?service=dr', active: true }).catch(() => {});
          });
        } else {
          heroSmsTopStatsHint.textContent = msg;
        }
      }
      return;
    }

    const enriched = Array.isArray(response.enriched) ? response.enriched : [];
    const statsApi = (typeof self !== 'undefined' && self.PhoneSmsHeroSmsStats)
      || (typeof globalThis !== 'undefined' && globalThis.PhoneSmsHeroSmsStats)
      || {};
    const fmtCost = typeof statsApi.formatCost === 'function'
      ? statsApi.formatCost
      : (v) => (Number.isFinite(v) && v > 0 ? `$${v.toFixed(4)}` : '-');
    const fmtPercent = typeof statsApi.formatPercent === 'function'
      ? statsApi.formatPercent
    ...
    // 排序状态 —— 默认按单价升序。点列标题切换：同列再点反向，换列重置 asc。
    // 「成功率」和「物理库存」用降序更直观（高的在前）。
    let sortKey = 'cost';
    let sortDir = 'asc'; // 'asc' | 'desc'
    const DEFAULT_DIRECTIONS = { cost: 'asc', successPercent: 'desc', physicalCount: 'desc', virtualCount: 'desc', countryId: 'asc' };

    const sortRows = (rows, key, dir) => {
      const factor = dir === 'desc' ? -1 : 1;
      // 物理库存=0 永远排在最下面（不论排序方式），避免误导用户
    ...
    const fetchedAt = response.fetchedAt
      ...
      : '';
    const meta = `service=${escapeHtml(response.service || 'dr')} · 近 ${response.interval || 12}h · ${response.successCount || 'medium'} 成功 · 拉取于 ${fetchedAt}`;

    const render = () => {
      const sorted = sortRows(enriched, sortKey, sortDir);
      const headerCell = (key, label, align = 'left') => {
        const active = key === sortKey;
        const arrow = active ? (sortDir === 'asc' ? ' ▲' : ' ▼') : '';
        const sortable = Boolean(DEFAULT_DIRECTIONS[key]);
        const cursor = sortable ? 'cursor:pointer;' : '';
        const color = active ? 'color:#06f;' : '';
        return `<th data-sort-key="${escapeHtml(key)}" style="padding:3px 6px; text-align:${align}; ${cursor}${color} user-select:none;" title="${sortable ? '点击切换排序' : ''}">${escapeHtml(label)}${arrow}</th>`;
      };
      const renderRow = (row, idx) => {
        const danger = row.unavailable;
        const rowStyle = danger ? 'background:#fff0f0;' : '';
        const stockCell = danger
          ? `<span style="color:#c00;" title="当前所有运营商所有档位均无空闲号">0 ⚠️</span>`
          : String(row.physicalCount);
        // 用 tier 明细做 tooltip：让用户悬停看到「$0.025 @ op_b: 10 个 / $0.075 @ op_a: 50 个 ...」
        const tierTooltip = Array.isArray(row.tierBreakdown) && row.tierBreakdown.length
          ? row.tierBreakdown.map((t) => `${fmtCost(t.cost)} @ ${t.operator || 'any'}: ${t.count}`).join('\n')
          : (row.offerLookupFailed ? '档位数据拉取失败，回退到起价' : '当前没有可买档位');
        const costCellLabel = row.offerLookupFailed
          ? `<span style="color:#aa6;">${fmtCost(row.cost)}*</span>`
          : fmtCost(row.cost);
        const opLabel = row.bestOperator ? ` <span style="color:#888; font-size:10px;">@${escapeHtml(row.bestOperator)}</span>` : '';
        return `<tr style="${rowStyle}" title="${escapeHtml(tierTooltip)}">
          <td style="padding:2px 6px;">${idx + 1}</td>
          ${headerCellOmit('countryId', row.countryId)}
          <td style="padding:2px 6px;">${escapeHtml(row.chn)} / ${escapeHtml(row.eng)}</td>
          <td style="padding:2px 6px; text-align:right;">${fmtPercent(row.successPercent)}</td>
          <td style="padding:2px 6px; text-align:right;">${costCellLabel}${opLabel}</td>
          <td style="padding:2px 6px; text-align:right;">${stockCell}</td>
        </tr>`;
      };
      // 占位：ID 列内容；和 header 配对就行
      function headerCellOmit(_key, value) {
        return `<td style="padding:2px 6px;">${value}</td>`;
      }
      heroSmsTopStatsResult.innerHTML = `
        <div style="font-size:11px; color:#888; margin-bottom:4px;">${meta}（点列标题排序，可买总数=0 沉底；悬停看运营商+档位明细，价格带 * 表示档位数据拉取失败，回退到起价）</div>
        <table style="width:100%; font-size:11px; border-collapse:collapse;">
          <thead><tr style="background:#f2f2f7;">
            <th style="padding:3px 6px; text-align:left;">#</th>
            ${headerCell('countryId', 'ID', 'left')}
            <th style="padding:3px 6px; text-align:left;">国家</th>
            ${headerCell('successPercent', '成功率', 'right')}
            ${headerCell('cost', '实付价', 'right')}
            ${headerCell('physicalCount', '可买总数', 'right')}
          </tr></thead>
          <tbody>${sorted.map((r, i) => renderRow(r, i)).join('')}</tbody>
        </table>
      `;
      // 绑定 header 点击
      heroSmsTopStatsResult.querySelectorAll('th[data-sort-key]').forEach((th) => {
        th.addEventListener('click', () => {
          const key = th.getAttribute('data-sort-key');
          if (!key || !DEFAULT_DIRECTIONS[key]) return;
          if (key === sortKey) {
            sortDir = sortDir === 'asc' ? 'desc' : 'asc';
          } else {
            sortKey = key;
            sortDir = DEFAULT_DIRECTIONS[key];
          }
          render();
        });
      });
    ...
    render();
    heroSmsTopStatsResult.style.display = '';
    if (heroSmsTopStatsHint) {
      heroSmsTopStatsHint.style.color = '#888';
      const unavailableCount = enriched.filter((r) => r.unavailable).length;
      heroSmsTopStatsHint.textContent = `已就绪：Top10 共 ${enriched.length} 条${unavailableCount > 0 ? `（${unavailableCount} 条物理库存为 0 已沉底）` : ''}。`;
    }
  } catch (err) {
    if (heroSmsTopStatsHint) {
      heroSmsTopStatsHint.style.color = '#c00';
      heroSmsTopStatsHint.textContent = `请求失败：${err?.message || err}`;
    }
  } finally {
    btnHeroSmsTopStats.disabled = false;
    btnHeroSmsTopStats.textContent = originalLabel;
  }
});
