...
test('phone signup step 2 does not treat chatgpt auth login entry as logged-in home', async () => {
  const logs = [];
  const messages = [];
  const completedPayloads = [];
  let reopenedEntry = false;
  ...
  const executor = step2Api.createStep2Executor({
    ...
    completeNodeFromBackground: async (nodeId, payload) => {
      completedPayloads.push({ nodeId, payload });
    },
    ensureContentScriptReadyOnTab: async () => {},
    ensureSignupAuthEntryPageReady: async () => {
      reopenedEntry = true;
    ...
    sendToContentScriptResilient: async (_source, message) => {
      ...
      if (message.type === 'ENSURE_SIGNUP_PHONE_ENTRY_READY') {
        return { ready: true, state: 'phone_entry', url: 'https://chatgpt.com/auth/login' };
      }
    ...
    OPENAI_AUTH_INJECT_FILES: [],
    waitForTabStableComplete: async () => ({ id: 17, url: 'https://chatgpt.com/auth/login' }),
  });

  await executor.executeStep2({
    signupMethod: 'phone',
    signupPhoneNumber: '5553984850130',
  });

  assert.deepStrictEqual(messages, [
    'ENSURE_SIGNUP_ENTRY_READY',
    'ENSURE_SIGNUP_PHONE_ENTRY_READY',
    'EXECUTE_NODE',
  ]);
  assert.equal(reopenedEntry, false);
  assert.equal(logs.some(({ message }) => /已登录 ChatGPT 首页/.test(message)), false);
  assert.equal(completedPayloads[0]?.nodeId, 'submit-signup-email');
  assert.equal(completedPayloads[0]?.payload.accountIdentifierType, 'phone');
  assert.equal(completedPayloads[0]?.payload.nextSignupState, 'password_page');
});
...
test('step 2 phone signup throws PHONE_RESEND_BANNED_NUMBER when landing on /log-in/password', async () => {
  const completedPayloads = [];
  const cancelled = [];
  const activation = {
    activationId: 'banned-activation',
  ...
  const executor = step2Api.createStep2Executor({
    ...
    ensureContentScriptReadyOnTab: async () => {},
    ensureSignupEntryPageReady: async () => ({ tabId: 21 }),
    ensureSignupPostIdentityPageReadyInTab: async () => ({
      state: 'password_page',
      url: 'https://auth.openai.com/log-in/password',
    }),
    ...
    phoneVerificationHelpers: {
      ...
      cancelSignupPhoneActivation: async (_state, act) => {
        cancelled.push(act);
      },
    },
    resolveSignupMethod: () => 'phone',
  ...
  await assert.rejects(
    ...
    (err) => {
      assert.ok(/^PHONE_RESEND_BANNED_NUMBER::/.test(err.message), `expected banned-number prefix, got: ${err.message}`);
      assert.ok(/573237462004/.test(err.message));
      assert.ok(/\/log-in\/password/.test(err.message));
      return true;
    }
  );

  assert.deepStrictEqual(completedPayloads, []);
  assert.deepStrictEqual(cancelled, [activation]);
});

test('step 2 phone signup still completes when landing on /create-account/password', async () => {
  const completedPayloads = [];
  const activation = {
    activationId: 'ok-activation',
  ...
  const executor = step2Api.createStep2Executor({
    ...
    ensureContentScriptReadyOnTab: async () => {},
    ensureSignupEntryPageReady: async () => ({ tabId: 22 }),
    ensureSignupPostIdentityPageReadyInTab: async () => ({
      state: 'password_page',
      url: 'https://auth.openai.com/create-account/password',
    }),
    ...
    phoneVerificationHelpers: {
      prepareSignupPhoneActivation: async () => activation,
      cancelSignupPhoneActivation: async () => {
        throw new Error('activation should not be cancelled on healthy signup password page');
      },
    },
    resolveSignupMethod: () => 'phone',
    resolveSignupEmailForFlow: async () => {
  ...
  assert.equal(completedPayloads[0].step, 'submit-signup-email');
  assert.equal(completedPayloads[0].payload.accountIdentifier, '447900000001');
  assert.equal(completedPayloads[0].payload.nextSignupUrl, 'https://auth.openai.com/create-account/password');
});
