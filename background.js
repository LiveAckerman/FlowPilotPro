...
const HERO_SMS_SUPPORTED_COUNTRY_IDS = [6, 52, 187, 16, 151, 43, 73, 78, 10];
...
function isRetryableContentScriptTransportError(error) {
  ...
  return /back\/forward cache|message channel is closed|Receiving end does not exist|port closed before a response was received|A listener indicated an asynchronous response|Frame with ID \d+ is showing error page|Cannot access contents of (?:the page|url)|chrome-error:\/\/chromewebdata|内容脚本\s+\d+(?:\.\d+)?\s*秒内未响应|did not respond in \d+s|failed to fetch|networkerror|network error|fetch failed|load failed/i.test(message);
}
...
function isBindEmailAddressInUseFailure(error) {
  const message = getErrorMessage(error);
  // 内容脚本统一用 STEP8_EMAIL_IN_USE:: 前缀（add-email 阶段就是这么抛的），
  // bind-email/fetch-bind-email-code 也会抛同样前缀；中英文 fallback 也兜一下，
  // 以防错误是从其他路径冒出来的纯文案。
  return /STEP8_EMAIL_IN_USE::|email(?:_already)?_in_use|该邮箱(?:地址)?(?:已有关联(?:账户)?|已被(?:使用|绑定|占用))/i.test(message);
}

function isSignupUnsupportedCountryFailure(error) {
  const message = getErrorMessage(error);
  // OpenAI 在区域受限时返回 unsupported_country。这是 IP/VPN 层面的硬性拒绝，
  // 任何重试、换号、换邮箱都无法绕过，必须当场停整轮自动运行并提示用户切换 VPN。
  return /SIGNUP_UNSUPPORTED_COUNTRY::|unsupported_country|OpenAI\s*(?:'s)?\s*服务在你所在的(?:国家|地区)(?:\s*[\\\/、，,]\s*(?:国家|地区))?(?:暂时)?不可用|OpenAI(?:'s|s)?\s*(?:services?|service)(?:\s+(?:is|are))?\s+not\s+available\s+in\s+your\s+(?:country|region|location)/i.test(message);
}
...
async function ensureAutoEmailReady(targetRun, totalRuns, attemptRuns) {
  ...
  if (currentState.email) {
    // 把「复用 state.email」这条路径显式打到日志里，方便定位「为什么 round 2 没去 Duck/iCloud 拿新邮箱」。
    // 正常情况下新一轮第一次尝试时 state.email 应该已经被 auto-run-controller 的 sanity 守卫清空；
    // 如果这里命中且 attemptRuns === 1，说明本轮根本没走 useExistingProgress 之外的清零路径，是泄漏；
    // 如果 attemptRuns > 1，则是同一轮内重试沿用，属于预期行为。
    const generatorLabel = typeof getEmailGeneratorLabel === 'function'
      ? getEmailGeneratorLabel(normalizeEmailGenerator(currentState.emailGenerator))
      : String(currentState.emailGenerator || 'duck');
    await addLog(
      `=== 目标 ${targetRun}/${totalRuns} 轮：检测到 state.email 已有值（${currentState.email}），直接复用，不再调 ${generatorLabel} 生成新邮箱（第 ${attemptRuns} 次尝试）===`,
      attemptRuns > 1 ? 'info' : 'warn'
    );
...
async function runAutoSequenceFromNodeGraph(startNodeId, context = {}) {
  ...
  let bindEmailInUseRestartCount = 0;
  const BIND_EMAIL_IN_USE_MAX_RESTARTS = 3;
  ...
  if (await shouldRunNamedNode('submit-signup-email')) {
    try {
      ...
      // 手机号注册时若 OpenAI 把号码识别为已存在账号、跳到登录密码页（/log-in/password），
      // submit-signup-email 会抛 PHONE_RESEND_BANNED_NUMBER 或 SIGNUP_PHONE_PASSWORD_MISMATCH。
      // 走与 fill-password / fetch-signup-code 相同的换号重跑链路，回到 open-chatgpt。
      const isPhoneResendBannedAtStep2 = typeof phoneVerificationHelpers !== 'undefined'
      ...
      if (isSignupPhonePasswordMismatchFailure(err) || isPhoneResendBannedAtStep2) {
        step4RestartCount += 1;
        await restartSignupPhonePasswordMismatchAttemptFromNode('submit-signup-email', step4RestartCount, err);
  ...
  while (nodeIndex < nodeIds.length) {
    ...
    try {
      ...
      // bind-email / fetch-bind-email-code 报「该邮箱地址已有关联账户 / email_already_in_use」时，
      // 丢弃当前注册邮箱、清空 bindEmailSubmitted 标记并回到 bind-email 重新生成邮箱再绑。
      // 不动手机身份（preserveAccountIdentity=true），因为手机号已经成功完成注册，只是绑邮箱失败。
      if (
        (nodeId === 'bind-email' || nodeId === 'fetch-bind-email-code')
        && typeof isBindEmailAddressInUseFailure === 'function'
        && isBindEmailAddressInUseFailure(err)
      ) {
        bindEmailInUseRestartCount += 1;
        if (bindEmailInUseRestartCount > BIND_EMAIL_IN_USE_MAX_RESTARTS) {
          await addLog(
            `节点 ${nodeId}：检测到绑定邮箱已被占用（email_already_in_use），已连续重试 ${bindEmailInUseRestartCount - 1} 次仍失败，停止自动重试。原因：${getErrorMessage(err)}`,
            'error'
          );
          throw err;
        }
        const stateBeforeReroll = await getState();
        const previousEmail = String(stateBeforeReroll?.email || '').trim();
        const previousSuffix = previousEmail ? `当前邮箱：${previousEmail}；` : '';
        await addLog(
          `节点 ${nodeId}：检测到 OpenAI 拒绝当前绑定邮箱（email_already_in_use），将清空当前注册邮箱并重新生成后回到节点 bind-email 重试（第 ${bindEmailInUseRestartCount}/${BIND_EMAIL_IN_USE_MAX_RESTARTS} 次）。${previousSuffix}原因：${getErrorMessage(err)}`,
          'warn'
        );
        if (typeof setEmailStateSilently === 'function') {
          await setEmailStateSilently('', {
            preserveAccountIdentity: true,
            preservePrevious: true,
            source: 'bind_email_in_use_reroll',
          });
        } else {
          await setState({
            email: '',
        ...
        // 关键：清完状态还不够 —— 当前 auth 标签页还停在 /email-verification 的
        // email_already_in_use 错误页，bind-email 重启时会立刻因为「页面不是 add_email_page」
        // 又抛错。必须把标签页导航回 OAuth URL，让 OpenAI 把流量重新导到 /add-email，
        // bind-email 重启时 resolveSignupEmailForFlow 才能进入「邮箱为空 → 调 fetchDuckEmail
        // 拿新地址」的分支，然后正常提交新邮箱完成绑定。
        const oauthUrlForReroll = String(stateBeforeReroll?.oauthUrl || '').trim();
        if (oauthUrlForReroll && typeof reuseOrCreateTab === 'function') {
          try {
            await reuseOrCreateTab('openai-auth', oauthUrlForReroll, { activate: true });
            await addLog(
              `节点 ${nodeId}：已把认证页导航回 OAuth URL，等待 OpenAI 重定向到添加邮箱页后再次生成新邮箱。`,
              'info'
            );
          } catch (navError) {
            await addLog(
              `节点 ${nodeId}：导航认证页回 OAuth URL 失败（${getErrorMessage(navError)}），bind-email 重启可能会再次因为页面状态不对而失败。`,
              'warn'
            );
          }
        } else if (!oauthUrlForReroll) {
          await addLog(
            `节点 ${nodeId}：缺少 oauthUrl，无法导航回 OAuth URL，bind-email 重启可能会因为当前页面仍停在错误页而再次失败。`,
            'warn'
          );
        }
        await invalidateDownstreamAfterAutoRunNodeRestart('bind-email', {
          logLabel: `节点 ${nodeId} 因 email_already_in_use 回到 bind-email 换邮箱重试（第 ${bindEmailInUseRestartCount}/${BIND_EMAIL_IN_USE_MAX_RESTARTS} 次）`,
        });
        nodeIndex = Math.max(0, getNodeIndex(await getState(), 'bind-email'));
        continue;
      }

      // 全局兜底：任何节点抛出 SIGNUP_PHONE_PASSWORD_MISMATCH 或 PHONE_RESEND_BANNED_NUMBER 都视为
      // 「当前注册手机号已被 OpenAI 识别为已存在账号 / 接码号无法收码」，统一走「丢弃当前手机号 →
      // 回到 open-chatgpt 重新获取手机号并重跑注册」的恢复链路，与 fill-password / fetch-signup-code
      // 既有的换号重跑路径对齐。（用 typeof 守卫以兼容测试 harness 未注入这些 helper 的场景。）
      const isPhoneResendBannedAtNode = typeof phoneVerificationHelpers !== 'undefined'
        && typeof phoneVerificationHelpers?.isPhoneResendBannedNumberError === 'function'
        && phoneVerificationHelpers.isPhoneResendBannedNumberError(err);
      const isSignupPhoneMismatchAtNode = typeof isSignupPhonePasswordMismatchFailure === 'function'
        && isSignupPhonePasswordMismatchFailure(err);
      if (
        (isSignupPhoneMismatchAtNode || isPhoneResendBannedAtNode)
        && typeof restartSignupPhonePasswordMismatchAttemptFromNode === 'function'
      ) {
        step4RestartCount += 1;
        await restartSignupPhonePasswordMismatchAttemptFromNode(nodeId, step4RestartCount, err);
        nodeIndex = Math.max(0, getNodeIndex(await getState(), 'open-chatgpt'));
        continue;
      }
...
const messageRouter = self.MultiPageBackgroundMessageRouter?.createMessageRouter({
  ...
  finalizeStep3Completion: async (completionPayload = {}) => {
    const currentState = await getState();
    const signupTabId = await getTabId('openai-auth');
